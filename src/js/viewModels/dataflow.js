        ...
        function getReplionFlow(dep,dep_url) {
          ...
          $.ajax({
              ...
              success: function (data) {
                ...
                self.NodeExt.push({'id' : key});
                }
                
                for(var key in data[0]['nodepmp'] ){
                  self.NodePmp.push({'id' : key });
                  }

                  for(var key in data[0]['noderep'] ){
                    self.NodeRep.push({'id' : key});
                    }

                    self.NodeRep.valueHasMutated();
                    self.NodeData.valueHasMutated();

                 for (var i = 0; i < data[1].length; i++) {
                      self.LinkData.push({'start' : data[1][i].start,'category' :data[1][i].category ,'end' :data[1][i].end,'id' : dep + i});
