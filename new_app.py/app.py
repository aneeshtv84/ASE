...
from flaskr.license import load_license
...
api_url = config.get('AGENT_CONFIG', 'API_URL')
llm_model_id = config.get('AGENT_CONFIG', 'LLM_MODEL_ID')
api_token = config.get('AGENT_CONFIG', 'API_TOKEN')
llm_temperature = config.get('AGENT_CONFIG', 'LLM_TEMPERATURE')
llm_max_tokens = config.get('AGENT_CONFIG', 'LLM_MAX_TOKENS')
api_chat_id = config.get('AGENT_CONFIG', 'API_CHAT_ID')
api_prompt_prefix = config.get('AGENT_CONFIG', 'API_PROMPT_PREFIX')
api_timeout = config.get('AGENT_CONFIG', 'API_TIMEOUT')

license_path = os.path.join(agent_base,'.license.json')
...
collection_name="sybase_oracle"
llm_model_kwargs = json.loads(llm_model_kwargs)

def call_copilot(prompt: str, model: str = llm_model_id, temperature: float = llm_temperature, max_tokens: int = llm_max_tokens):
    API_URL = api_url
    API_TOKEN = api_token
    ...
    payload = {
        "prompt": prompt,
        "chat_type": "classic_chat",
        "chat_id": api_chat_id,
        "prompt_prefix": api_prompt_prefix,
        "model": model,
        "temperature": str(temperature),
        "max_tokens": str(max_tokens),
        "opt_out": "yes",
        "language": "english",
        "constitutional_ai_princinple": "",  
        "system_message": "",
        "summary_required": "no",
    }

    try:
        resp = requests.post(API_URL, headers=headers, data=json.dumps(payload), timeout=api_timeout)
    ...
    if isinstance(data, dict):
        ddl = data.get("response") or data.get("output") or data.get("data") or json.dumps(data)
        return ddl
    
    return str(data)

llm = RunnableLambda(lambda prompt: call_copilot(str(prompt)))
...
class GetConfigFile(Resource):
    def get(self):
        ...
        if not db_row:
            return jsonify({"error": "Config file not found in DB"})
    ...
    def post(self):
        ...
        try:
           if os.path.exists(config_file_path):
                ...
                backup_file = os.path.join(config_dir, f'skyliftai_{timestamp}.cfg')
    ...
    def post(self):
        ...
        for query in constraintDDL:
            if query.strip():
               ...
               match = re.search(r"`([^`]*)`", oracle_converted_sql, re.DOTALL)
               if match:
                   oracle_converted_sql = match.group(1).strip()
...
def convertCodeGenAi(sql_query):
    ...
    translate = get_sql_rag_chain(llm, retriever, strict_prompt_template, context, output_parser)
    max_retries = 5
...
class procDDLGenAi(Resource):
    ...
    def post(self):
        ...
        for chunk in chunks_with_headers:
            ...
            time.sleep(random.randint(5, 10))
        match = re.search(r"```(.*?)```", original_converted_sql, re.DOTALL)
        if match:
            original_converted_sql = match.group(1).strip()
...
class tableDDLGenAi(Resource):
    ...
    def post(self):
        ...
        match = re.search(r"(According to the rules[\s\S]*?```[\s\S]*?```)", oracle_converted_sql, re.IGNORECASE)
        if match:
            oracle_converted_sql = match.group(1).strip()
            match = re.search(r"```(?:\w+)?\s*([\s\S]*?)\s*```", oracle_converted_sql)
            if match:
                oracle_converted_sql = match.group(1).strip()
        else:
            oracle_converted_sql = oracle_converted_sql.strip()
...
class ggILAction(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            pass
        try:
            ILREP = "select distinct b.dep_url url_tgt,b.dbtype db_type , user, passwd  from onepconn b where b.dep_type='rd'"
            ILREP_fetch = pd.read_sql_query(ILREP, conn)
            for rep in ILREP_fetch.iterrows():
                url_tgt = rep[1]['url_tgt']
            api_url = url_tgt + '/ILREPFetch'
            ...
            try:
                ...
                if isinstance(data, list) and len(data) > 0:
                    ilOpData = data[0]
                    repDet = ilOpData.get("repDet", [])
                    tgtdomain = ilOpData.get("tgtdomain")
                    tgtalias = ilOpData.get("tgtalias")
                else:
                    raise ValueError("No data received from ILREPFetch")
            except (requests.exceptions.ConnectionError, requests.exceptions.Timeout, requests.exceptions.JSONDecodeError) as e:
                ILException = str(e)
                repDet, tgtdomain, tgtalias = [], None, None

            api_url = url_tgt + '/ggiljobact'
...
class CRTshoot(Resource):
    ...
    def post(self):
        ...
        con = pyodbc.connect(user, passwd, servicename)
        conn.close()
...
class getDeploymentData(Resource):
    def post(self):
        data = request.get_json(force=True)
        ...
        try:
            conn = sqlite3.connect('conn.db')
            cursor = conn.cursor()
            if dep_type in ('ld', 'rd'):
                cursor.execute("SELECT * FROM onepconn WHERE dep_type = ?", (dep_type,))
                rows = cursor.fetchall()
                return jsonify({"rows": rows})
            else:
                return {"error": "Invalid dep_type, must be 'ld' or 'rd'."}, 400
        except sqlite3.DatabaseError as e:
            return {"error": "There is a problem with OneP Database: " + str(e)}
        finally:
            if conn:
                conn.close()
            ...
            if dep_type == 'ld':
                ...
                try:
                    ...
                    if SignIn == 'Y' and oneplace_role == 'admin':
                        if onepOps == 'add':
                            cursor.execute('insert into ONEPCONN values(:dep,:user,:passwd,:role,:dep_type, :dep_url,:dbtype)', {'dep': oneplace_dep, 'user': user, 'passwd': passwd, 'role': oneplace_role, 'dep_type': dep_type, 'dep_url': dep_url, 'dbtype': dbtype})
                        elif onepOps == 'edit':                            
                            cursor.execute('update ONEPCONN set dep_url=:dep_url where dep_type=:dep_type',{'dep_type': 'rd', 'dep_url': dep_url})
                        elif onepOps == 'del':
                            cursor.execute('delete from  ONEPCONN  where dep_type=:dep_type', {'dep_type': 'rd'})
                        conn.commit()
                        ErrPrint.append('OneP User ' + user + ' Added')
                    else:
...
def get_sql_rag_chain(llm, retriever, strict_prompt_template, context, output_parser):
    rag_chain = create_stuff_documents_chain(llm=llm, prompt=strict_prompt_template, output_parser=output_parser)
...
def get_chroma_handles():
    if 'retriever' not in g:
        # Fresh client in the current thread/worker
        client = PersistentClient(path=str(vector_db_path), settings=chroma_settings)
        try:
            col = client.get_collection(collection_name)
        except NotFoundError:
            col = client.create_collection(collection_name)

        # LangChain wrapper using the same persist dir & embeddings
        db = LCChroma(
        ...
        # Cache on the request context
        g.chroma_client = client
        g.chroma_collection = col
        g.db = db
        g.retriever = retriever
    return g.retriever, g.chroma_client, g.chroma_collection , g.db
...
class TablePromptView(Resource):
    def get(self):
        try:
            file_path = os.path.join(RAG_PATH, 'table_ddl_prompt')
...
class ProcPromptView(Resource):
    def get(self):
        try:
            file_path = os.path.join(RAG_PATH, 'procedure_prompt')
...
class CodePromptView(Resource):
    def get(self):
        try:
            file_path = os.path.join(RAG_PATH, 'code_prompt')
...
class saveTablePrompt(Resource):
    def post(self):
        data = request.get_json(force=True)
        new_prompt = data['prompt']
        target_file = os.path.join(RAG_PATH , 'table_ddl_prompt')
        backup_dir = os.path.join(RAG_PATH, 'TableDDlPromptBackups')

        try:
            os.makedirs(backup_dir, exist_ok=True)
            if os.path.exists(target_file):
                timestamp = datetime.now().strftime('%d%m%Y_%H%M%S')
                backup_file = os.path.join(backup_dir, f'table_ddl_prompt_{timestamp}')
                shutil.copy2(target_file, backup_file)
            with open(target_file, 'w', encoding='utf-8') as f:
...
class saveProcPrompt(Resource):
    def post(self):
        data = request.get_json(force=True)
        new_prompt = data['prompt']
        target_file = os.path.join(RAG_PATH , 'procedure_prompt')
        backup_dir = os.path.join(RAG_PATH, 'ProcDDlPromptBackups')

        try:
            os.makedirs(backup_dir, exist_ok=True)
            if os.path.exists(target_file):
                timestamp = datetime.now().strftime('%d%m%Y_%H%M%S')
                backup_file = os.path.join(backup_dir, f'procedure_prompt_{timestamp}')
                shutil.copy2(target_file, backup_file)
            with open(target_file, 'w', encoding='utf-8') as f:
                f.write(new_prompt.strip())
            return {"message": "Saved successfully"}, 200
...
class saveCodePrompt(Resource):
    def post(self):
        data = request.get_json(force=True)
        new_prompt = data['prompt']
        target_file = os.path.join(RAG_PATH , 'code_prompt')
        backup_dir = os.path.join(RAG_PATH, 'codePromptBackups')

        try:
            os.makedirs(backup_dir, exist_ok=True)
            if os.path.exists(target_file):
                timestamp = datetime.now().strftime('%d%m%Y_%H%M%S')
                backup_file = os.path.join(backup_dir, f'code_prompt_{timestamp}')
                shutil.copy2(target_file, backup_file)
            with open(target_file, 'w', encoding='utf-8') as f:
                f.write(new_prompt.strip())
            return {"message": "Saved successfully"}, 200
...
class saveChromaDB(Resource):
    def post(self):
        ...
        try:
            ...
            saveClaudeFromat.main(RAG_PATH,chroma_settings)            
...
api.add_resource(getDeploymentData, '/getDeploymentData')
...
api.add_resource(SaveConfigFile, '/SaveConfigFile')
...
api.add_resource(CodePromptView, '/CodePromptView')
api.add_resource(ProcPromptView, '/ProcPromptView')
api.add_resource(TablePromptView, '/TablePromptView')
api.add_resource(savePattern, '/savePattern')
api.add_resource(saveTablePrompt, '/saveTablePrompt')
api.add_resource(saveProcPrompt, '/saveProcPrompt')
api.add_resource(saveCodePrompt, '/saveCodePrompt')
